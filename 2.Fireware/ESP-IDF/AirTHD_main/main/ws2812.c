...
void ws2812_init(int gpioNum, unsigned int getCounts)
{
  counts = getCounts;
...
{
  unsigned int i;
  ws2812_len = (counts * 3) * sizeof(uint8_t);
  ws2812_buffer = malloc(ws2812_len);

  for (i = 0; i < counts; i++)
  {
    ws2812_buffer[0 + i * 3] = r;
    ws2812_buffer[1 + i * 3] = g;
    ws2812_buffer[2 + i * 3] = b;
  }
...
void ws2812_setColors(rgbValue *array)
{
...
void ws2812_setCounts(unsigned int length)
{
    counts = length;
}
